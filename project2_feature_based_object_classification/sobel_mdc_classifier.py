...
def merge(i, j, ii, jj, pl):  # it does not perform merging between regions which should follow later.
    ...
    if reg[ii][jj][0] != pl:  # in case of second merging after merging once with upper or left pixel.
        ...
        if reginfo[int(old)][0] <= 0:
            ...
            reginfo[int(old)][1] = (reginfo[int(old)][1] * (reginfo[int(old)][0] + 1) - reg[ii][jj][1]) / \
                                   reginfo[int(old)][0]
...
def mergeregion(l1, l2):
    ...
    if l1 == l2 or reginfo[l1][1] - reginfo[l2][1] > th:
        ...
        reginfo[m][1] = (reginfo[m][1] * reginfo[m][0] + reginfo[n][1] * reginfo[n][0]) / (
                    reginfo[m][0] + reginfo[n][0])
...
cimg, row, col = readimage(f"o3.png")
inimg = np.full((row, col), 0)
# gray scaling & intlimit
color3dtogray2d(cimg, inimg)
intlimitimg(inimg)

writeimage2d(inimg, "1.png")
...
reg = np.full((row, col, 2),
              0.0)  # reg[i][j][0] is label value of pixel (i,j), and reg[i][j][1] is updated intensity of pixel (i,j)
reginfo = np.full((row * col, 2),
                  0.0)  # reginfo[i][0] is the pixel count of region label i, and reginfo[i][1] is updated intensity of region label i
    ...
    for j in range(col):  # upper first and left next makes a better result as low label is prioritized.
        ...
        if j != 0 and abs(reg[i][j][1] - reginfo[int(reg[i][j - 1][0])][1]) <= th:
            if rowmerge != 1:
                ...
                if reg[i - 1][j][0] != reg[i][j - 1][
                    0]:  # to exclude duplicate merging for the same label upper and left pixels
                    if abs(reg[i][j][1] - reginfo[int(reg[i][j - 1][0])][1]) <= th:
                        merge(i, j, i, j - 1,
                              reg[i - 1][j][0])  # if rowmerge already happened, pl is set to previous rowmerge
                        colmerge = 1

            if i != 0 and rowmerge == 0 and abs(reginfo[int(reg[i][j][0])][1] - reginfo[int(reg[i - 1][j][0])][
                1]) <= th * 0.5:  # After colmerge, if rowmerge becomes possible.
...
outregl2 = picklarge(outregl, reginfo, 1)
